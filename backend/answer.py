...
FASL = {
    ...
    "properties": {
        "faqarat": {
            ...
            "description": (
                "**كل عنصر من هذه القائمة فقرة تامة** فيها ثلاث جمل أو أربع مجتمعة، "
                "لا جملة واحدة. الجملة الأولى: العنصر ودلالته. ثم مستند الدلالة. ثم "
                "ما يعنيه للرائي. خاطب الرائي بضمير المخاطب، ولا تسرد كلامه سطراً سطراً."
            ),
        },
        "manhaj": {
            "type": "string",
...
٤- **تكلّم على الرؤيا كلاماً متصلاً في `tahlil_mufassal` قبل أن تُفرد الرموز.**
   ...
   - **وبناء الفقرة على هذا الترتيب، ولا تخرج عنه:**
     (أ) تبدأ **بالعنصر ودلالته في جملة خبرية جازمة**، لا بحكاية ما جرى.
     (ب) ثم مستند الدلالة: ما في الكتب، أو السنة، أو القراءة النفسية.
     (ج) ثم ما يعنيه ذلك لهذا الرائي في يقظته.
     ثلاث جمل أو أربع، **قصيرة**، ولا تصل الجمل بالواو حتى تطول.
     **والثلاث جمل كلها في عنصر واحد من `faqarat`** — فالعنصر فقرة تامة،
     لا جملة واحدة؛ فلا تجعل (أ) عنصراً و(ب) عنصراً و(ج) عنصراً.

     نموذج على المطلوب:
     «تكرار فكرتك أنك داخل حلم دليل على يقظة ذهنية عالية في أثناء المنام.
      وهذا مما يذكره أهل التعبير في أضغاث الأحلام، إذ يبقى العقل يقظاً
      يراقب. وهو في حالك يدل على نفس تحليلية تحاول الإمساك بزمام أمرها
      حتى في أضيق المواضع.»

     ومثال ما **لا** يُقبل — حكاية ثم تعليق مبهم:
     «تستيقظ في منامك وأنت على جانبك الأيمن معتقداً أنك نجوت، ثم تعود لك
      محاولات الانفصال، وفي كتاب كذا يُشار إلى أن ما يراه الإنسان من أمور
      ثقيلة قد يكون من تخاليط النوم، وتعيش أنت هذا الشعور بواقعية مخيفة
      تعكس ما يجول في خاطرك من هواجس.»
     فهذه أعادت عليه رؤياه وهو يعلمها، ثم لم تُفدْه شيئاً.
