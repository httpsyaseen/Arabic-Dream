...
def scrape_toc(book: books.Book, session: requests.Session) -> list[dict]:
...
def parse_page(html: str, book: books.Book) -> dict | None:
    soup = BeautifulSoup(html, "lxml")
...
def scrape_book(book: books.Book, start: int, limit: int | None, refetch: bool) -> None:
    session = requests.Session()
    out_dir = RAW / book.slug
    out_dir.mkdir(parents=True, exist_ok=True)
    TOC.mkdir(parents=True, exist_ok=True)

    toc_path = TOC / f"{book.slug}.json"
